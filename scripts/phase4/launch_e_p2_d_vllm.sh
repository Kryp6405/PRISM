...
###############################################################################
# Phase 4 E/P/D native vLLM launch script — NO RAY, REPLICATED PREFILL
#
# Layout:
#   NODE0 GPUs 0,1     -> Encoder vLLM        TP=2
#   NODE0 GPU  2       -> Prefill-0 vLLM      TP=1
#   NODE0 GPU  3       -> Prefill-1 vLLM      TP=1
#   NODE1 GPUs 0,1,2,3 -> Decode vLLM         TP=4
#
# Total active GPUs: 8
#
# Proxy:
...
#
# Key idea:
#   Avoid TP>1 inside Prefill, because multimodal embedding merge breaks under
#   TP in this vLLM build. Use two TP=1 Prefill replicas instead.
###############################################################################
...
PREFILL0_PORT="${PREFILL0_PORT:-19535}"
PREFILL1_PORT="${PREFILL1_PORT:-19537}"
...
PREFILL0_NIXL_SIDE_CHANNEL_PORT="${PREFILL0_NIXL_SIDE_CHANNEL_PORT:-5559}"
PREFILL1_NIXL_SIDE_CHANNEL_PORT="${PREFILL1_NIXL_SIDE_CHANNEL_PORT:-5560}"
...
PREFILL0_CUDA_VISIBLE_DEVICES="${PREFILL0_CUDA_VISIBLE_DEVICES:-2}"
PREFILL1_CUDA_VISIBLE_DEVICES="${PREFILL1_CUDA_VISIBLE_DEVICES:-3}"
...
PREFILL_URLS="http://localhost:${PREFILL0_PORT},http://localhost:${PREFILL1_PORT}"

log "=== Phase 4 E/P/D native vLLM launch: replicated Prefill ==="
log "MODEL=$MODEL"
log "PORT=$PORT"
log "ENCODE_PORT=$ENCODE_PORT"
log "PREFILL0_PORT=$PREFILL0_PORT"
log "PREFILL1_PORT=$PREFILL1_PORT"
...
log "PREFILL_NODE=$PREFILL_NODE"
log "  Prefill-0 GPU $PREFILL0_CUDA_VISIBLE_DEVICES  TP=$PREFILL_TP_SIZE PP=$PREFILL_PP_SIZE"
log "  Prefill-1 GPU $PREFILL1_CUDA_VISIBLE_DEVICES  TP=$PREFILL_TP_SIZE PP=$PREFILL_PP_SIZE"
...
log "PREFILL0_NIXL_SIDE_CHANNEL_HOST=$PREFILL_IP  PORT=$PREFILL0_NIXL_SIDE_CHANNEL_PORT"
log "PREFILL1_NIXL_SIDE_CHANNEL_HOST=$PREFILL_IP  PORT=$PREFILL1_NIXL_SIDE_CHANNEL_PORT"
...

{
  ...
  echo "  $PREFILL_NODE  GPU  $PREFILL0_CUDA_VISIBLE_DEVICES      -> Prefill-0 TP=$PREFILL_TP_SIZE PP=$PREFILL_PP_SIZE"
  echo "  $PREFILL_NODE  GPU  $PREFILL1_CUDA_VISIBLE_DEVICES      -> Prefill-1 TP=$PREFILL_TP_SIZE PP=$PREFILL_PP_SIZE"
  echo "  $DECODE_NODE   GPUs $DECODE_CUDA_VISIBLE_DEVICES        -> Decode    TP=$DECODE_TP_SIZE PP=$DECODE_PP_SIZE"
  echo
  echo "No Ray. Prefill is scaled by replicas, not TP."
  echo "Proxy prefill URLs: $PREFILL_URLS"
...
export PREFILL0_PORT="$PREFILL0_PORT"
export PREFILL1_PORT="$PREFILL1_PORT"
export PREFILL_PORT="$PREFILL0_PORT"
export PREFILL_URLS="$PREFILL_URLS"
export DECODE_PORT="$DECODE_PORT"
export MODEL="$MODEL"
export VLLM_ROOT="$VLLM_ROOT"
export EC_SHARED_STORAGE_PATH="$EC_SHARED_STORAGE_PATH"
export PREFILL0_NIXL_SIDE_CHANNEL_PORT="$PREFILL0_NIXL_SIDE_CHANNEL_PORT"
export PREFILL1_NIXL_SIDE_CHANNEL_PORT="$PREFILL1_NIXL_SIDE_CHANNEL_PORT"
...
export PREFILL0_CUDA_VISIBLE_DEVICES="$PREFILL0_CUDA_VISIBLE_DEVICES"
export PREFILL1_CUDA_VISIBLE_DEVICES="$PREFILL1_CUDA_VISIBLE_DEVICES"
...
###############################################################################
# Step 1 - Encoder on Node 0 GPUs 0,1
###############################################################################
...
###############################################################################
# Step 2a - Prefill-0 on Node 0 GPU2
###############################################################################

log "=== Step 2a: Prefill-0 vLLM on $PREFILL_NODE GPU $PREFILL0_CUDA_VISIBLE_DEVICES TP=$PREFILL_TP_SIZE ==="

ssh -q "$PREFILL_NODE" "
$(remote_env_prefix)

export VLLM_NIXL_SIDE_CHANNEL_HOST='$PREFILL_IP'
export VLLM_NIXL_SIDE_CHANNEL_PORT='$PREFILL0_NIXL_SIDE_CHANNEL_PORT'

CUDA_VISIBLE_DEVICES='$PREFILL0_CUDA_VISIBLE_DEVICES' vllm serve '$MODEL' \
  --host 0.0.0.0 \
  --port '$PREFILL0_PORT' \
...
PREFILL0_PID=$!
log "Prefill-0 PID=$PREFILL0_PID"

wait_for_local_http "Prefill-0 backend" "$PREFILL0_PORT" "$PREFILL0_PID"

###############################################################################
# Step 2b - Prefill-1 on Node 0 GPU3
###############################################################################

log "=== Step 2b: Prefill-1 vLLM on $PREFILL_NODE GPU $PREFILL1_CUDA_VISIBLE_DEVICES TP=$PREFILL_TP_SIZE ==="

ssh -q "$PREFILL_NODE" "
$(remote_env_prefix)

export VLLM_NIXL_SIDE_CHANNEL_HOST='$PREFILL_IP'
export VLLM_NIXL_SIDE_CHANNEL_PORT='$PREFILL1_NIXL_SIDE_CHANNEL_PORT'

CUDA_VISIBLE_DEVICES='$PREFILL1_CUDA_VISIBLE_DEVICES' vllm serve '$MODEL' \
  --host 0.0.0.0 \
  --port '$PREFILL1_PORT' \
  --tensor-parallel-size '$PREFILL_TP_SIZE' \
  --pipeline-parallel-size '$PREFILL_PP_SIZE' \
...
wait_for_local_http "Prefill-1 backend" "$PREFILL1_PORT" "$PREFILL1_PID"

###############################################################################
# Step 3 - Decode on Node 1 GPUs 0,1,2,3
###############################################################################

log "=== Step 3: Decode vLLM on $DECODE_NODE GPUs $DECODE_CUDA_VISIBLE_DEVICES TP=$DECODE_TP_SIZE ==="
...

{
  ...
  echo "PREFILL0_SHELL_PID=$PREFILL0_PID"
  echo "PREFILL1_SHELL_PID=$PREFILL1_PID"
  ...
  echo "PREFILL0_CUDA_VISIBLE_DEVICES=$PREFILL0_CUDA_VISIBLE_DEVICES"
  echo "PREFILL1_CUDA_VISIBLE_DEVICES=$PREFILL1_CUDA_VISIBLE_DEVICES"
  ...
  echo "PREFILL0_PORT=$PREFILL0_PORT"
  echo "PREFILL1_PORT=$PREFILL1_PORT"
  echo "PREFILL_URLS=$PREFILL_URLS"
  echo "DECODE_PORT=$DECODE_PORT"
  echo "PORT=$PORT"
  echo "PREFILL0_NIXL_SIDE_CHANNEL_PORT=$PREFILL0_NIXL_SIDE_CHANNEL_PORT"
  echo "PREFILL1_NIXL_SIDE_CHANNEL_PORT=$PREFILL1_NIXL_SIDE_CHANNEL_PORT"
...
wait "$ENCODER_PID" "$PREFILL0_PID" "$PREFILL1_PID" "$DECODE_PID" "$PROXY_PID"
