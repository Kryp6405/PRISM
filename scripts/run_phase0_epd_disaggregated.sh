...
# Phase 0 runner for full E/P/D disaggregation.
# Exact launch commands are supplied via environment variables.

: "${EPD_FRONTEND_CMD:?Set EPD_FRONTEND_CMD to the frontend launch command}"
: "${EPD_ENCODER_CMD:?Set EPD_ENCODER_CMD to the encoder worker launch command}"
: "${EPD_PREFILL_CMD:?Set EPD_PREFILL_CMD to the prefill worker launch command}"
: "${EPD_DECODE_CMD:?Set EPD_DECODE_CMD to the decode worker launch command}"
...
RUN_PREFIX="${RUN_PREFIX:-${MODEL//\//_}_phase0_epd}"
...
bash -lc "$EPD_FRONTEND_CMD" > "$LOG_DIR/frontend.log" 2>&1 &
FRONTEND_PID=$!

echo "Launching encoder worker"
bash -lc "$EPD_ENCODER_CMD" > "$LOG_DIR/encoder.log" 2>&1 &
ENCODER_PID=$!

echo "Launching prefill worker"
bash -lc "$EPD_PREFILL_CMD" > "$LOG_DIR/prefill.log" 2>&1 &
PREFILL_PID=$!

echo "Launching decode worker"
bash -lc "$EPD_DECODE_CMD" > "$LOG_DIR/decode.log" 2>&1 &
DECODE_PID=$!
...
for _ in $(seq 1 150); do
  if grep -q "Application startup complete" "$LOG_DIR/encoder.log" 2>/dev/null || \
     grep -q "Application startup complete" "$LOG_DIR/prefill.log" 2>/dev/null || \
     grep -q "Application startup complete" "$LOG_DIR/decode.log" 2>/dev/null || \
     grep -q "Registered endpoint 'generate'" "$LOG_DIR/decode.log" 2>/dev/null; then
    READY=1
    break
  fi
  sleep 2
...
if [[ "$READY" -ne 1 ]]; then
  echo "E/P/D workers failed to become ready; inspect logs in $LOG_DIR" | tee "$RUN_DIR/summary.txt"
  exit 1
fi

for CONCURRENCY in $CONCURRENCY_VALUES; do
  OUT_DIR="$ARTIFACT_ROOT/${MODEL//\//_}-epd-openai-chat-concurrency${CONCURRENCY}"
...
python3 src/phase0/summarize_run.py \
  ...
  --run-prefix "${MODEL//\//_}-epd-openai-chat-concurrency" \
...
echo "Phase 0 E/P/D run complete."
