...
def sort_key(row: dict[str, Any]) -> tuple:
    ...
    return (
        generation_order.get(generation, 99),
        c,
...
def aggregate_run_count_by_generation(rows: list[dict[str, Any]]) -> dict[str, int]:
    out: dict[str, int] = {}
    for r in rows:
        g = r.get("generation") or "unknown"
        out[g] = out.get(g, 0) + 1
    return out
...
def main() -> None:
    ...
    parser.add_argument("--parser-script", type=Path, default=Path("src/phase2/parse_aiperf.py"))
    ...
    output = {
        ...
        "counts_by_generation": aggregate_run_count_by_generation(rows),
