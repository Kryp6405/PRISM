...
def parse_profile_export_aiperf_json(path: Path) -> dict[str, Any]:
    ...
    out["input_sequence_length_p99"] = get_stat(isl, "p99")
    out["input_sequence_length_min"] = get_stat(isl, "min")
    out["input_sequence_length_max"] = get_stat(isl, "max")
...
def parse_gpu_telemetry_from_aiperf_json(path: Path, mode: str | None) -> dict[str, Any]:
    ...
    for idx, role in role_map.items():
        ...
        add_role_aliases(out, role)

    add_gpu_skew_and_active_count(out, used_indices)
...
def add_gpu_aliases(out: dict[str, Any]) -> None:
...
def add_role_aliases(out: dict[str, Any], role: str) -> None:
...
def add_gpu_skew_and_active_count(out: dict[str, Any], used_indices: list[int]) -> None:
